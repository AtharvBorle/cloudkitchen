    ...
    const fetchBusinessCategories = async () => {
        try {
            ...
            if (res.ok) {
                ...
                const foodCats = categories.filter((c: any) => c.type === "FOOD");
                setBusinessCategories(foodCats);
                if (foodCats.length > 0) {
                    setSelectedParentId(foodCats[0].id);
                }
            }
        } catch (error) {
    ...
    const handleCreateCategoryFromTicket = async (reqDetails: any) => {
        ...
        try {
            if (reqDetails.type === "Food Category") {
                ...
                if (!parentId) {
                    throw new Error("Please select a parent category first.");
                }
                
                const selectedParent = businessCategories.find(c => c.id === parentId);
                const parentName = selectedParent ? selectedParent.name : "Selected Parent";
                                                     ...
                                                     <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: "10px" }}>
                                                         ...
                                                         {reqDetails.type === "Food Category" && (
                                                             <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                                                                 <strong style={{ color: "#475569" }}>Select Parent Category:</strong>
                                                                 <select
