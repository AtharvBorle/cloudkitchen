...
import { Star, MessageSquare, Search, Filter, RefreshCw, ThumbsUp, CheckCircle, Tag, TrendingUp } from "lucide-react";
...
export default function SuperadminReviewsPage() {
    ...
    const loadReviews = async () => {
        setLoading(true);
        try {
            const res = await fetchApi("/api/superadmin/reviews");
            if (res.ok) {
            ...
            console.error("Failed to load superadmin reviews:", err);
        } finally {
            setLoading(false);
        }
    };
        ...
        return (
            <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "60vh" }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "10px" }}>
                    ...
                    <span style={{ color: "var(--text-muted)", fontWeight: 500 }}>Loading Ratings &amp; Reviews...</span>
                </div>
            </div>
        );
    }
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "24px", maxWidth: "1200px", margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "16px" }}>
                <div>
                    ...
                    <p style={{ color: "var(--text-muted)", fontSize: "0.875rem", marginTop: "4px" }}>
                        Monitor and analyze user sentiment, star ratings, and feedback across the platform.
                    </p>
                </div>
                <button
                    onClick={loadReviews}
            ...
            {stats && (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "16px" }}>
                    <div style={{ backgroundColor: "white", padding: "20px", borderRadius: "12px", border: "1px solid var(--border)", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
                        ...
                        <div style={{ fontSize: "0.75rem", color: "#10B981", marginTop: "4px", display: "flex", alignItems: "center", gap: "4px" }}>
                            <TrendingUp size={14} /> Recorded from web and app users
                        </div>
                    </div>
                    ...
                    <div style={{ backgroundColor: "white", padding: "20px", borderRadius: "12px", border: "1px solid var(--border)", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
                        <div style={{ color: "var(--text-muted)", fontSize: "0.875rem", fontWeight: 500 }}>Top Liked Aspect</div>
                        <div style={{ fontSize: "1.2rem", fontWeight: "700", color: "var(--primary)", marginTop: "8px", textOverflow: "ellipsis", overflow: "hidden", whiteSpace: "nowrap" }}>
                            {Object.entries(stats.aspectCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || "Food Taste & Quality"}
                        </div>
                        <div style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginTop: "4px" }}>
                            Most frequently selected highlight
                        </div>
                    </div>

                    <div style={{ backgroundColor: "white", padding: "20px", borderRadius: "12px", border: "1px solid var(--border)", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
                        <div style={{ color: "var(--text-muted)", fontSize: "0.875rem", fontWeight: 500 }}>Top Quick Tag</div>
                        <div style={{ fontSize: "1.2rem", fontWeight: "700", color: "#10B981", marginTop: "8px", textOverflow: "ellipsis", overflow: "hidden", whiteSpace: "nowrap" }}>
                            {Object.entries(stats.tagCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || "Super Fast Delivery"}
                        </div>
                        <div style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginTop: "4px" }}>
                            Most popular customer tag
                        </div>
                    </div>
                </div>
            )}
            ...
            {/* Filter & Search Bar */}
            <div style={{ backgroundColor: "white", padding: "16px 20px", borderRadius: "12px", border: "1px solid var(--border)", display: "flex", flexWrap: "wrap", gap: "16px", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                {filteredReviews.length === 0 ? (
                    <div style={{ backgroundColor: "white", padding: "40px", borderRadius: "12px", border: "1px solid var(--border)", textAlign: "center", color: "var(--text-muted)" }}>
