...
export default function SupportTicketsPage() {
    ...
    const [selectedBusinessCategoryIds, setSelectedBusinessCategoryIds] = useState<string[]>([]);
    const [selectedFoodCategoryId, setSelectedFoodCategoryId] = useState<string>("CREATE_NEW");
    const [newFoodCategoryName, setNewFoodCategoryName] = useState<string>("");
    const [subcategoryName, setSubcategoryName] = useState<string>("");

    const fetchBusinessAndFoodCategories = async () => {
        try {
            // 1. Fetch public categories (for business categories, type "FOOD")
            ...
            const fcRes = await fetchApi("/api/superadmin/food-categories");
            if (fcRes.ok) {
                const data = await fcRes.json();
                const payload = data.data || data;
                setFoodCategories(payload.foodCategories || []);
            }
        } catch (error) {
    ...
    useEffect(() => {
        fetchBusinessAndFoodCategories();
    }, []);

    useEffect(() => {
        if (selectedTicket && selectedTicket.category === "NEW_CATEGORY_REQUEST") {
    ...
    const handleCreateCategoryFromTicket = async (reqDetails: any) => {
        ...
        try {
            if (reqDetails.type === "Food Category") {
                let targetFoodCategoryId = selectedFoodCategoryId;

                // 1. If we are creating a new food category
                if (selectedFoodCategoryId === "CREATE_NEW") {
                    if (!newFoodCategoryName.trim()) {
                        throw new Error("Please enter a new food category name.");
                    }
                    if (selectedBusinessCategoryIds.length === 0) {
                        throw new Error("Please select at least one parent business category.");
                    }

                    const formData = new FormData();
                    formData.append("name", newFoodCategoryName.trim());
                    formData.append("categoryIds", selectedBusinessCategoryIds.join(","));
                    ...
                    const data = await res.json();
                    if (!res.ok) {
                        throw new Error(data.message || data.error || "Failed to create food category.");
                    }
                    
                    const payload = data.data || data;
                    const foodCategory = payload.foodCategory || payload;
                    targetFoodCategoryId = foodCategory.id;
                }

                // 2. If subcategory name is typed, create the subcategory
                if (subcategoryName.trim()) {
                    if (!targetFoodCategoryId) {
                        throw new Error("Parent food category ID is missing.");
                    }

                    const subFormData = new FormData();
                    subFormData.append("name", subcategoryName.trim());
                    subFormData.append("foodCategoryId", targetFoodCategoryId);

                    const subRes = await fetch("/api/superadmin/food-subcategories", {
                        method: "POST",
                        body: subFormData
                    });
                    ...
                    if (!subRes.ok) {
                        throw new Error(subData.message || subData.error || "Failed to create food sub-category.");
                    }
                    
                    setCategoryCreatedMsg(`Successfully created! Food Category: "${newFoodCategoryName.trim() || 'Selected Parent'}", Subcategory: "${subcategoryName.trim()}"`);
                } else {
                    if (selectedFoodCategoryId === "CREATE_NEW") {
                        setCategoryCreatedMsg(`Food Category "${newFoodCategoryName.trim()}" successfully created!`);
                    } else {
                        throw new Error("Please specify a subcategory name to create under the selected food category.");
                    }
                }

                await fetchBusinessAndFoodCategories();
                                                 ...
                                                 <div style={{ backgroundColor: "white", padding: "12px", borderRadius: "8px", border: "1px solid #E2E8F0", fontSize: "0.75rem" }}>
                                                     ...
                                                     <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: "10px" }}>
                                                         ...
                                                         {reqDetails.type === "Food Category" && (
                                                             <>
                                                                 <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: "10px" }}>
                                                                     <strong style={{ color: "#475569" }}>Select Business Categories (Multiple):</strong>
                                                                     <div style={{
                                                                         display: "flex",
                                                                         ...
                                                                         {businessCategories.length === 0 ? (
                                                                             <div style={{ color: "#94A3B8" }}>No categories available</div>
                                                                         ) : (
                                                                             businessCategories.map((c: any) => {
                                                                                 const isChecked = selectedBusinessCategoryIds.includes(c.id);
                                                                                 return (
                                                                                     <label key={c.id} style={{ display: "flex", alignItems: "center", gap: "6px", cursor: "pointer", userSelect: "none" }}>
                                                                                         <input
                                                                                             type="checkbox"
                                                                                             checked={isChecked}
                                                                                             onChange={() => {
                                                                                                 if (isChecked) {
                                                                                                     setSelectedBusinessCategoryIds(prev => prev.filter(id => id !== c.id));
                                                                                                 } else {
                                                                                                     setSelectedBusinessCategoryIds(prev => [...prev, c.id]);
                                                                                                 }
                                                                                             }}
                                                                                         />
                                                                                         <span style={{ fontSize: "0.75rem", color: "#334155" }}>{c.name}</span>
                                                                                     </label>
                                                                                 );
                                                                             })
                                                                         )}
                                                                 ...
                                                                 <div style={{ display: "flex", flexDirection: "column", gap: "4px", marginBottom: "10px" }}>
                                                                     <strong style={{ color: "#475569" }}>Parent Category (e.g. shake, cake):</strong>
                                                                     <select
                                                                         value={selectedFoodCategoryId}
                                                                         onChange={(e) => setSelectedFoodCategoryId(e.target.value)}
                                                                         style={{
                                                                             width: "100%",
                                                                             padding: "6px",
                                                                             borderRadius: "6px",
                                                                     ...
                                                                     >
                                                                         <option value="CREATE_NEW">+ Create New Food Category...</option>
                                                                         {foodCategories.map((fc: any) => (
                                                                 ...
                                                                 {selectedFoodCategoryId === "CREATE_NEW" && (
                                                                     <div style={{ display: "flex", flexDirection: "column", gap: "4px", marginBottom: "10px" }}>
                                                                         <strong style={{ color: "#475569" }}>New Food Category Name:</strong>
                                                                         <input
                                                                             type="text"
                                                                             value={newFoodCategoryName}
                                                                             onChange={(e) => setNewFoodCategoryName(e.target.value)}
                                                                             placeholder="Enter food category (e.g. Shake)"
                                                                             style={{
                                                                                 width: "100%",
                                                                                 padding: "6px",
                                                                                 borderRadius: "6px",
                                                                 ...
                                                                 <div style={{ display: "flex", flexDirection: "column", gap: "4px", marginBottom: "10px" }}>
                                                                     <strong style={{ color: "#475569" }}>Sub-Category (Optional; e.g. badam shake):</strong>
                                                                     <input
                                                                         type="text"
